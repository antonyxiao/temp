...
{
    assert(val != NULL);
    node_t *n = emalloc(sizeof(node_t));
    n->next = NULL;
    n->val = val;
    return n;
}
...
node_t *add_inorder(node_t *list, node_t *new)
{
    ...
    // the first event's start date is later than the new event's start date
    if (list == NULL || strcmp(list->val->start_date, new->val->start_date) > 0)
    {
    ...
    {
        if (strcmp(cur->next->val->start_date, new->val->start_date) > 0)
        {
            cur->next = add_front(cur->next, new);
            return list;
        } // if
    } // for
